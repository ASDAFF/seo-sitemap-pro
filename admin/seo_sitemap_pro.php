...
while($sitemap = $data->NavNext())
{
	...
	$row->AddActions(array(
		array(
			...
			"ACTION" => $adminList->ActionRedirect("seo_sitemap_pro_edit.php?ID=".$sitemap["ID"]."&lang=".LANGUAGE_ID),
			"DEFAULT" => true,
		),
		array(
			"ICON" => "move",
...
foreach($arSites as $arRes)
{
	$arDDMenu[] = array(
		...
		"LINK" => "seo_sitemap_pro_edit.php?lang=".LANGUAGE_ID."&site_id=".$arRes['LID']
	);
}
...
BX.runSitemap = function(ID, value, pid, NS)
{
	...
	BX.ajax.post('/bitrix/admin/seo_sitemap_pro_run.php', {
		lang:'<?=LANGUAGE_ID?>',
		action: 'sitemap_run',
		ID: ID,
		value: value,
...
{
	window.tbl_sitemap.GetAdminList('/bitrix/admin/seo_sitemap_pro.php?lang=<?=LANGUAGE_ID?>');
};
</script>
