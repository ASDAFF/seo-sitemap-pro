...
if($ID > 0)
{
	...
	else
	{
		if($_REQUEST['action'] == 'delete' && check_bitrix_sessid())
		{
			...
			LocalRedirect(BX_ROOT."/admin/seo_sitemap_pro.php?lang=".LANGUAGE_ID);
		}
...
$aTabs = array(
	array("DIV" => "seo_sitemap_pro_common", "TAB" => Loc::getMessage('SEO_SITEMAP_COMMON'), "ICON" => "main_settings", "TITLE" => Loc::getMessage('SEO_SITEMAP_COMMON_TITLE')),
	array("DIV" => "seo_sitemap_pro_files", "TAB" => Loc::getMessage('SEO_SITEMAP_FILES'), "ICON" => "main_settings", "TITLE" => Loc::getMessage('SEO_SITEMAP_FILES_TITLE')),
);
if($bIBlock)
{
	$aTabs[] = array("DIV" => "seo_sitemap_pro_iblock", "TAB" => Loc::getMessage('SEO_SITEMAP_IBLOCK'), "ICON" => "main_settings", "TITLE" => Loc::getMessage('SEO_SITEMAP_IBLOCK_TITLE'));
}
if($bForum)
{
	$aTabs[] = array("DIV" => "seo_sitemap_pro_forum", "TAB" => Loc::getMessage('SEO_SITEMAP_FORUM'), "ICON" => "main_settings", "TITLE" => Loc::getMessage('SEO_SITEMAP_FORUM_TITLE'));
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid() && (strlen($_POST["save"]) > 0 || strlen($_POST['apply']) > 0 || strlen($_POST['save_and_add']) > 0))
{
	...
	if(empty($errors))
	{
		$arSitemapSettings = SitemapTable::prepareSettings(array(
            ...
            // PRIORITY
            'DEFAULT_FILES_PRIORITY' => $_REQUEST['DEFAULT_FILES_PRIORITY'],
            'DEFAULT_IBLOCK_PRIORITY' => $_REQUEST['DEFAULT_IBLOCK_PRIORITY'],
            'DEFAULT_FORUM_PRIORITY' => $_REQUEST['DEFAULT_FORUM_PRIORITY'],
            'IBLOCK_PRIORITY' => $_REQUEST['IBLOCK_PRIORITY'],
            'IBLOCK_SECTION_PRIORITY' => $_REQUEST['IBLOCK_SECTION_PRIORITY'],
            'FORUM_TOPIC_PRIORITY' => $_REQUEST['FORUM_TOPIC_PRIORITY'],
		));
		...
		if($result->isSuccess())
		{
			...
			{
				LocalRedirect(BX_ROOT."/admin/seo_sitemap_pro.php?lang=".LANGUAGE_ID."&run=".$ID."&".bitrix_sessid_get());
			}
			else
			{
				LocalRedirect(BX_ROOT."/admin/seo_sitemap_pro_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&".$tabControl->ActiveTabParam());
			}
		}
		else
		{
...
function seo_getIblock($arSettings = array(), $iblockId, $sectionId, $sectionChecked, $elementChecked, $arSectionChecked = array(), $arElementChecked = array())
{
...
<table class="internal" style="width: 100%;">
	<tr class="heading">
        ...
        <td width="120"><?=Loc::getMessage('SITEMAP_PRIORITY')?></td>
	</tr>
<?
			}
?>
	<tr>
        ...
        <td align="center"><input style="width: 30px; text-align: center;" type="text" maxlength="3" name="IBLOCK_SECTION_PRIORITY[<?=$iblockId?>][<?=$d?>]" value="<?= $arSettings['IBLOCK_SECTION_PRIORITY'][$iblockId][$d] ?>"></td>
	</tr>
...
$aMenu[] = array(
	...
	"LINK"	=> "/bitrix/admin/seo_sitemap_pro.php?lang=".LANGUAGE_ID,
	"ICON"	=> "btn_list",
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".Loc::getMessage("SITEMAP_DELETE_CONFIRM")."')) window.location='/bitrix/admin/seo_sitemap_pro_edit.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
		"ICON"	=> "btn_delete",
...
<tr class="adm-detail-required-field">
    <td width="40%"><?=Loc::getMessage("SITEMAP_DEFAULT_PRIORITY")?>:</td>
    <td width="60%"><input type="text" name="DEFAULT_FILES_PRIORITY" value="<?=Converter::getHtmlConverter()->encode($arSitemap['SETTINGS']["DEFAULT_FILES_PRIORITY"])?>" maxlength="3" style="width:30px;text-align:center;"></td>
</tr>
<tr>
...
<tr class="adm-detail-required-field">
    <td width="40%"><?=Loc::getMessage("SITEMAP_DEFAULT_PRIORITY")?>:</td>
    <td width="60%"><input type="text" name="DEFAULT_IBLOCK_PRIORITY" value="<?=Converter::getHtmlConverter()->encode($arSitemap['SETTINGS']["DEFAULT_IBLOCK_PRIORITY"])?>" maxlength="3" style="width:30px;text-align:center;"></td>
</tr>
...
?>
			<tr>
                ...
                <td align="center"><input style="width: 30px; text-align: center;" type="text" maxlength="3" name="IBLOCK_PRIORITY[<?=$d?>]" value="<?= $arSitemap['SETTINGS']['IBLOCK_PRIORITY'][$d]?>"></td>
			</tr>
			<tr style="display: none" id="subdirs_row_<?=$r?>">
<?
...
<tr class="adm-detail-required-field">
    ...
    <tr class="adm-detail-required-field">
        <td width="40%"><?=Loc::getMessage("SITEMAP_DEFAULT_PRIORITY")?>:</td>
        <td width="60%"><input type="text" name="DEFAULT_FORUM_PRIORITY" value="<?=Converter::getHtmlConverter()->encode($arSitemap['SETTINGS']["DEFAULT_FORUM_PRIORITY"])?>" maxlength="3" style="width:30px;text-align:center;"></td>
    </tr>
...
?>
			<tr>
                ...
                <td align="center"><input style="width: 30px; text-align: center;" type="text" maxlength="3" name="FORUM_TOPIC_PRIORITY[<?=$d?>]" value="<?= $arSitemap['SETTINGS']['FORUM_TOPIC_PRIORITY'][$d] ?>"></td>
			</tr>
<?
	}
