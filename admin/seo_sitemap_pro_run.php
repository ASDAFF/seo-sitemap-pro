...
use Maximaster\SeoSitemapPro\SitemapIndexEx;
use Maximaster\SeoSitemapPro\SitemapRuntimeEx;
...
Main\Loader::includeModule('maximaster.seositemappro');
			...
			if($dir['TYPE'] == 'F')
			{
					...
					|| $arSitemap['SETTINGS']['FILE'][$dirKey] == 'Y')
				{
					if(preg_match($arSitemap['SETTINGS']['FILE_MASK_REGEXP'], $dir['FILE']))
					{
						...
						$sitemapFile->addFileEntry($f, $arSitemap['SETTINGS']['DEFAULT_FILES_PRIORITY']);
						$NS['files_count']++;
					}
				}
			}
...
if($_REQUEST['action'] == 'sitemap_run' && check_bitrix_sessid())
{
	...
	if($v == $arValueSteps['init'])
	{
		...
		$sitemapFile = new SitemapRuntimeEx($PID, $arSitemap['SETTINGS']['FILENAME_FILES'], $arSitemapSettings);
			...
			if(count($arIBlockList) > 0)
			{
				...
				foreach($arIBlockList as $iblockId => $iblockActive)
				{
					...
					else
					{
						...
						$sitemapFile = new SitemapRuntimeEx($PID, $fileName, $arSitemapSettings);
			...
			elseif(is_array($arCurrentIBlock))
			{
				...
				if(isset($NS['CURRENT_SECTION']))
				{
					...
					if($arElement)
					{
                        ...
                        $priority = $arSitemap['SETTINGS']['IBLOCK_PRIORITY'][ $iblockId ]
                            ? $arSitemap['SETTINGS']['IBLOCK_PRIORITY'][ $iblockId ]
                            : $arSitemap['SETTINGS']['DEFAULT_IBLOCK_PRIORITY'];
				...
				else
				{
					...
					if($arSection)
					{
						...
						if($bActive)
						{
                            ...
                            $dbg = $arSitemap['SETTINGS']['IBLOCK_SECTION_PRIORITY'][ $iblockId ][ $arSection['ID'] ];

                            $priority = $arSitemap['SETTINGS']['IBLOCK_SECTION_PRIORITY'][ $iblockId ][ $arSection['ID'] ]
                                ? $arSitemap['SETTINGS']['IBLOCK_SECTION_PRIORITY'][ $iblockId ][ $arSection['ID'] ]
                                : ($arSitemap['SETTINGS']['IBLOCK_PRIORITY'][ $iblockId ]
                                    ? $arSitemap['SETTINGS']['IBLOCK_PRIORITY'][ $iblockId ]
                                    : $arSitemap['SETTINGS']['DEFAULT_IBLOCK_PRIORITY']);
			...
			elseif(is_array($arCurrentForum))
			{
				...
				if($bActive)
				{
					...
					if(($arTopic = $dbTopicResult->fetch()) && $arTopic)
					{
                        ...
                        $priority = $arSitemap['SETTINGS']['FORUM_TOPIC_PRIORITY'][ $forumId ]
                            ? $arSitemap['SETTINGS']['FORUM_TOPIC_PRIORITY'][ $forumId ]
                            : $arSitemap['SETTINGS']['DEFAULT_FORUM_PRIORITY'];
	...
	else
	{
		...
		$sitemapFile = new SitemapIndexEx($arSitemap['SETTINGS']['FILENAME_INDEX'], $arSitemapSettings);
